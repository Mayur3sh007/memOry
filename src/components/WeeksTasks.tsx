...
const WeeksTasks: React.FC = () => {
    ...
    const isWithinNext7Days = (date: string) => {
    ...
    const notesWithScheduleTimeNext7DaysWithImages = pendingNotes.filter(note =>
        note.ImageURL !== null && isWithinNext7Days(note.scheduleTime)
    );

    const notesWithScheduleTimeNext7DaysWithoutImages = pendingNotes.filter(note =>
        note.ImageURL === null && isWithinNext7Days(note.scheduleTime)
    );

    const sortedUpcomingNotesWithImages = notesWithScheduleTimeNext7DaysWithImages.sort((a, b) =>
        new Date(a.scheduleTime).getTime() - new Date(b.scheduleTime).getTime()
    );

    const sortedUpcomingNotesWithOutImages = notesWithScheduleTimeNext7DaysWithoutImages.sort((a, b) =>
        new Date(a.scheduleTime).getTime() - new Date(b.scheduleTime).getTime()
    );

    console.log(sortedUpcomingNotesWithImages);
    console.log(sortedUpcomingNotesWithOutImages);
    ...
    return (
        <>
            {(sortedUpcomingNotesWithImages.length > 0 || sortedUpcomingNotesWithOutImages.length > 0) && (
                <>
                    <h1 className="text-center text-2xl font-bold mt-16">My Week</h1>
                    <NotesCard
